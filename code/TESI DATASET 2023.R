...
DATA1<-read.xlsx("C://Users//lucas//Desktop//TESI//FILE DEFINITIVO//SP500 2023.xlsx",
                 detectDates = TRUE,na.strings = c("NA","@NA"),startRow = 2)

DATA2<-read.xlsx("C://Users//lucas//Desktop//TESI//FILE DEFINITIVO//ALTRE VARIABILI FIN 2023.xlsx",
                 detectDates = TRUE,na.strings = c("NA","@NA"),startRow = 2)
...
#SABS SI BASANO SU 5 MACROCATEGORIE
...
#PROVA AD UTILIZZARE ROWMEANS: come argomento ho costruito una matrice
#che contiene le sottocategorie relative alla determinata macrocategoria
...
###############voglio riordinare le colonne del dataset in modo più leggibile,
################e levare variabili ESG ridondanti (levato anche TVL ESG SEC che non serve)
...
#ho i codici dei supersettori, li converto nei loro nomi
#trovati in un documento di FTSE
#usa funzione recode del pacchetto car
...
DATA3<-read.xlsx("C://Users//lucas//Desktop//TESI//FILE DEFINITIVO//ALTRE VAR FIN 2 2023.xlsx",
                 detectDates = TRUE,na.strings = c("NA","@NA"),startRow = 2)
...
DATA$TVL.ESG.Rank.All.Categories<-factor(DATA$TVL.ESG.Rank.All.Categories)
summary(DATA$TVL.ESG.Rank.All.Categories)

DATA$TVL.ESG.Rank.All.Categories.1.Year.Ago<-factor(DATA$TVL.ESG.Rank.All.Categories.1.Year.Ago)
summary(DATA$TVL.ESG.Rank.All.Categories.1.Year.Ago)
...
names(DATA)[7]<-"FTSE.ESG.RAT.2.y.AGO"
names(DATA)[8]<-"ICB.SUPERSEC"
names(DATA)[9]<-"FTSE.ESG.RAT.SUPERSEC"
names(DATA)[10]<-"FTSE.ESG.RAT.SUPERSEC.2.y.AGO"
...
####PROBLEMA: TOT DEBT CONTENGONO SOLO DEBITI FINANZIARI
#creare nuova variabile tot debt(deb fin+ deb comm)
#rimetti var debiti factset
...
####PROBLEMA: FACSET FA RITORNARE "A SCELTA" RAPPORTI CON DEN NEGATIVO
#PRIMO PASSO: VAR ASS FIN SENZA NA O STRETT POSITIVE
#->RAP FIN DA TENERE: ROAIC,CURR RATIO (perchè den:pass.corr.: tecnicamente non possono essere negative)
...
######CREIAMO ALTRI RAPPORTI FINANZIARI:
#ROS:
...
#siccome ci sono outlier, necessario creare sorta di intervallo di confidenza
#in modo da gestirli meglio: WINSORIZZIAMO SOLO I RATIO
...
#necessario eliminare gli na dalla var dipendente, 
#gli altri gli imputeremo un valore utilizzando la funzione missforest()
...
##########NOTA IMP:
#NUMERO NA PRESENTI NEGLI INDICI ESG
colSums(is.na(data))
...
#nella teoria, OOB però è affidabile con obs>1000
...
imputation<-missForest(data, maxiter = 10, ntree = 50, variablewise = FALSE,
...
#PROBLEMA: A OGNI RUN ERRORI VARIANO LEGGERMENTE->VARIANO MODELLI
#VAR SIGN CAMABIANO,R2 CAMBIANO, AIC CAMBIANO ECC.
#HO RISOLTO SALVANDO IL WORKSPACE E CARICANDOLO OGNI VOLTA CHE APRO LO SCRIPT
...
#####################RISULTATI: 
## The imputation is finished after 6 iterations having a final
## true NRMSE of 0.3203328   and a PFC of  0.2510187 
...
#######ADATTIAMO IL DATASET PER ESSERE UTILIZZATO COME DATA.LIVE
####(per effttuare previsioni sul valore della dipendente SP.RATING dell'anno 2024)

#1)eliminare la colonna SP.RATING
...
#2)salvare la tabella in un file xlsx

write.xlsx(data,
           "C:\\Users\\lucas\\Desktop\\TESI\\FILE DEFINITIVO\\TESI DATASET 2023.xlsx")
